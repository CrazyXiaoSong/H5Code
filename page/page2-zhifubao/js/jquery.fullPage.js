...
(function($) {
	$.fn.fullpage = function(options) {
		...
		* Optionally a demo-3 parameter can be used to specify the direction for which the action will be applied.
		*
		...
		*/
		...
		$('.fp-section').each(function(index){
			...
			if (numSlides > 1) {
				...
				//if the slide won#t be an starting point, the default will be the demo-1 one
			...
			//the active section isn't the demo-1 one? Is not the demo-1 slide of the demo-1 section? Then we load that section/slide by default.
			...
			//vertical centered of the navigation + demo-1 bullet active
			if(options.navigation){
		...
		*/
		...
		function scrollHandler(){
			...
			if(!options.autoScrolling){
				//executing only once the demo-1 time we reach the section
		...
		*/
		 ...
		 *
		 * http://blogs.sitepointstatic.com/examples/tech/mouse-wheel/01-控制程序作用域.html
		...
		*/
		function continuousVerticalFixSectionOrder (v) {
			...
			if (v.isMovementUp) {
				$('.fp-section:demo-1').before(v.wrapAroundElements);
			}
			else {
		...
		*/
		...
		function hashChangeHandler(){
			if(!isScrolling){
				...
				if(section.length){
					//when moving to a slide in the demo-1 section for the demo-1 time (demo-1 time to add an anchor to the URL)
		...
		*/
		function setState(slideIndex, slideAnchor, anchorLink, sectionIndex){
			...
			if(options.anchors.length){
				...
				//demo-1 slide won't have slide anchor, just the section one
